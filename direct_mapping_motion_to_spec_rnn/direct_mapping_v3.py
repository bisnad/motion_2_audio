...
but for multiple mocap and audio files
"""
...
mocap_file_path = "D:/data/mocap/Motion2Audio/stocos"
mocap_files = [ "Take_1_50fps_crop.bvh", "Take_2_50fps_crop.bvh", "Take_3_50fps_crop.bvh", "Take_4_50fps_crop.bvh", "Take_6_50fps_crop.bvh" ]
mocap_fps = 50
...
audio_files = [ "Take1__double_Bind_HQ_audio_crop_16khz.wav", "Take2_Hibr_II_HQ_audio_crop_16khz.wav", "Take3_RO_37-4-1_HQ_audio_crop_16khz.wav", "Take4_Blumen_Baile_HQ_audio_crop_16khz.wav", "Take6_PhantomLimb_part3_HQ_audio_crop_16khz.wav" ]
audio_sample_rate = 16000
audio_n_hop_length = audio_sample_rate // mocap_fps
audio_n_win_length = audio_n_hop_length * 2
audio_n_mels = 128
audio_n_fft = 2048
audio_dim = audio_n_mels
...
# load model weights
load_weights = False
m2a_weights_file = "results_v1/weights/m2a_weights_epoch_600"
...
for mocap_data in all_mocap_data:
    ...
    all_mocap_sequences.append(mocap_sequence)

all_mocap_sequences_concat = np.concatenate(all_mocap_sequences, axis=0)

mocap_mean = np.mean(all_mocap_sequences_concat)
mocap_std = np.std(all_mocap_sequences_concat)

for i in range(len(all_mocap_sequences)):
    all_mocap_sequences[i] = (all_mocap_sequences[i] - mocap_mean) / mocap_std
...
# compute mel spectra of entire waveforms

all_mel_data = []

for waveform_data in all_waveform_data:

    mel_data = mel_transform(waveform_data)
    mel_data = mel_data.squeeze(axis=0)
    all_mel_data.append(mel_data)

# normalise mel spectra

all_mel_data_concat = torch.concat(all_mel_data, dim=1)

audio_data_mean = torch.mean(all_mel_data_concat, dim=1).unsqueeze(dim=1)
audio_data_std = torch.std(all_mel_data_concat, dim=1).unsqueeze(dim=1)

all_audio_data = []

for audio_data in all_mel_data:
...
# make sure audio and mocap data are of the same length

for i in range(len(all_mocap_sequences)):
    
    mocap_length = all_mocap_sequences[i].shape[0]
    audio_length = all_audio_data[i].shape[0]
    
    print("i mocap seq length ", mocap_length, " audio seq length ", audio_length)
    
    total_sequence_length = min(mocap_length, audio_length)
    
    all_mocap_sequences[i] = all_mocap_sequences[i][:total_sequence_length, ...]
    all_audio_data[i] = all_audio_data[i][:total_sequence_length, ...]
...
for i in range(len(all_mocap_sequences)):
    ...
    mocap_sequence = all_mocap_sequences[i]
    audio_sequence = all_audio_data[i]
...
mocap_excerpts.shape
audio_excerpts.shape
...
def create_ref_mocap_excerpt(excerpt_index, file_name):
    ...
    mocap_excerpt = mocap_excerpt_norm * mocap_std + mocap_mean
    mocap_excerpt = mocap_excerpt.reshape(1, sequence_length, joint_count, joint_dim).to(device)
...
def create_ref_audio_excerpt(excerpt_index, file_name):
    ...
    torchaudio.save(file_name, audio_excerpt, audio_sample_rate)
    
def create_pred_audio_excerpt(excerpt_index, file_name):
...
def create_ref_audio(start_frame, end_frame, file_name):
    mel_excerpt_norm = audio_sequence[start_frame:end_frame, :]
    mel_excerpt_norm = torch.permute(mel_excerpt_norm, (1, 0))
    mel_excerpt = mel_excerpt_norm * audio_data_std + audio_data_mean
...
def create_pred_audio(start_frame, end_frame, window_offset, file_name):
    ...
    mocap_region_norm = mocap_sequence[start_frame:end_frame]
    mocap_region_norm = mocap_region_norm.reshape(mocap_region_norm.shape[0], -1)
    ...
    torchaudio.save(file_name, audio_region, audio_sample_rate)  
        
# create ref mocap excerpt

test_excerpt_index = 1000
    
create_ref_mocap_excerpt(test_excerpt_index, "results/anims/ref_mocap_{}.gif".format(test_excerpt_index))

# create ref audio excerpt    

test_excerpt_index = 1000
    
create_ref_audio_excerpt(test_excerpt_index, "results/audio/ref_audio_{}.wav".format(test_excerpt_index))

# create pred audio excerpt    

test_excerpt_index = 1000
    
create_pred_audio_excerpt(test_excerpt_index, "results/audio/pred_audio_{}_epoch_{}.wav".format(test_excerpt_index, epochs))

# create ref mocap
    
seq_index = 4
mocap_sequence = torch.tensor(all_mocap_sequences[seq_index]).to(device)
test_start_frame = 1000
test_end_frame = 2000  

create_mocap_anim(test_start_frame, test_end_frame, "results/anims/ref_mocap_seq_{}_excerpt_{}-{}.gif".format(seq_index, test_start_frame, test_end_frame))

# create ref audio

seq_index = 4
audio_sequence = all_audio_data[seq_index]
test_start_frame = 1000
test_end_frame = 2000  
window_offset = 64

create_ref_audio(test_start_frame, test_end_frame, "results/audio/ref_audio_seq_{}_excerpt_{}-{}.wav".format(seq_index, test_start_frame, test_end_frame))
...
create_pred_audio(test_start_frame, test_end_frame, window_offset, "results/audio/pred_audio_seq_{}_excerpt_{}-{}_epoch_{}.wav".format(seq_index, test_start_frame, test_end_frame, epochs))


# test with different mocap file

def diff_create_ref_mocap(mocap_norm, start_frame, end_frame, offset, file_name):

    mocap_region_norm = mocap_norm[start_frame:end_frame]
    mocap_region = mocap_region_norm * mocap_std + mocap_mean
    mocap_region = mocap_region.reshape(1, mocap_region.shape[0], joint_count, joint_dim).to(device)
...
def diff_create_pred_audio(mocap_norm, start_frame, end_frame, window_offset, file_name):
...
diff_mocap_data_path = "D:/data/mocap/Motion2Audio/stocos/Take_7_50fps_crop.bvh"
...
diff_create_ref_mocap(diff_mocap_sequence, test_start_frame, test_end_frame, window_offset, "results/anims/diff_ref_mocap_{}-{}.gif".format(test_start_frame, test_end_frame))
diff_create_pred_audio(diff_mocap_sequence, test_start_frame, test_end_frame, window_offset, "results/audio/diff_gen_audio_{}-{}.wav".format(test_start_frame, test_end_frame))

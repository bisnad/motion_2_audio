"""
simple rnn that directly maps a sequence of dance poses into sequence of mels
the number of mels and dance poses must be identical with each mel having a corresponding dance pose
"""
...
audio_file = "D:/data/audio/Motion2Audio/improvisation_audio_16khz.wav"
...
# create audio data (standardize and transpose)
audio_data = mel_data.squeeze(axis=0)
audio_data_mean = torch.mean(audio_data, dim=1).unsqueeze(dim=1)
audio_data_std = torch.std(audio_data, dim=1).unsqueeze(dim=1)
audio_data = (audio_data - audio_data_mean) / audio_data_std
audio_data = torch.permute(audio_data, (1, 0))

# create audio sequence

audio_sequence = audio_data[valid_frame_start:valid_frame_end,...]
...
create_mocap_anim(test_start_frame, test_end_frame, "results/anims/ref_mocap_{}-{}.gif".format(test_start_frame, test_end_frame))
...
create_ref_audio(test_start_frame, test_end_frame, "results/audio/ref_audio_{}-{}.wav".format(test_start_frame, test_end_frame))
...
create_pred_audio(test_start_frame, test_end_frame, window_offset, "results/audio/pred_audio_{}-{}_epoch_{}.wav".format(test_start_frame, test_end_frame, epochs))
